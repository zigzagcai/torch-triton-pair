...
class Model(nn.Module):
    ...
    def forward(self, x, delta, A, B2, C, initial_state):
        return ref_selective_scan(x, delta, A, B2, C, initial_state)
...
def get_inputs():
    A = (-(torch.rand(D, K, dtype=dtype)).exp().to(device)).requires_grad_(True)
    x = torch.randn(B, T, D, dtype=dtype).to(device).requires_grad_(True)
    delta = torch.randn(B, T, D, dtype=dtype).sigmoid().to(device).requires_grad_(True)
    B2 = torch.randn(B, T, K, dtype=dtype).to(device).requires_grad_(True)
    C = torch.randn(B, T, K, dtype=dtype).to(device).requires_grad_(True)
    initial_state = torch.randn(B, D, K, dtype=dtype).to(device).requires_grad_(False)
    return [x, delta, A, B2, C, initial_state]
